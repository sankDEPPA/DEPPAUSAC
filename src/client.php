...
class APIClient
{
     ...
     *                             actualmente).
     */
    public function __construct($url_base, $usuario, $passwd, $unidad, $config)
    {
    ...
    {
        $dataSend = array_merge(
            $data,
            [
                'usuario' => $this->usuario,
                'password' => $this->passwd,
                'codigo_unidad' => $this->unidad
            ]
        );
            ...
            try {
                ...
                if (!empty($dataArray)) {
                    ...
                    return [
                        'error' => true,
                        'response' => $response,
                        'descripcion' => 'La respuesta no se encuentra bien formada.',
                        'errorCode' => static::ERROR_BAD_REQUEST,
                ...
                return [
                    ...
                    'descripcion' => 'Ha sucedido un error al intentar convertir la respuesta a un Array.',
                    'errorCode' => static::ERROR_BAD_RESPONSE
                ];
            }
        } catch (Exception $exc) {
            return [
                'error' => true,
                'exception' => $exc->getTraceAsString(),
                'descripcion' => 'Ha sucedido un error al intentar realizar la petición.',
                'errorCode' => static::ERROR_REQUEST
            ];
        }
    }
     ...
     * @param string $registro_personal (Opcional) Registro de personal del docente a evaluar - importante cuando la sección es compartida por varios docentes
     *
     * @return array
     */
    public function generarEnlaceEvaluacionEnLinea(
        $anio,
        $codigo_periodo,
        $codigo_curso,
        $seccion,
        ...
        if ($registro_personal !== '') {
            $data['registro_personal'] = $registro_personal;
        }
